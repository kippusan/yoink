...
pub(crate) async fn find_or_create_lightweight_artist(
    ...
    // if let Ok(Some(id)) =
    //     db::find_artist_by_provider_link(&state.sqlite, provider, artist_external_id).await
    // {
    //     return Ok(id);
    // }
    ...
    //     created_at: Utc::now(),
    // };
    // db::upsert_artist(&state.sqlite, &artist).await?;
    // {
    ...
    //     image_ref: None,
    // };
    // db::upsert_artist_provider_link(&state.sqlite, &link).await?;
